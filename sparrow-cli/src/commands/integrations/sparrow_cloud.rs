...
impl<'a> SparrowManager<'a> {
    ...
    pub(crate) async fn deploy(
        ...
        // Build a pruned SparrowConfig containing only [project] and the deployed [cloud.<instance>]
        ...
        if let Some(instance_name) =
            self.project
                ...
                .find_map(|(instance_name, cloud_config)| match cloud_config {
                    CloudConfig::SparrowCloud(config) if config.cluster_id == cluster_id => {
