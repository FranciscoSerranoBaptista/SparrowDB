...
use crate::sparrowc::generator::return_values::RustFieldType;
use crate::sparrowc::generator::utils::{GenRef, RustType};
use crate::sparrowc::{
    analyzer::{
        Ctx,
...
fn process_object_literal<'a>(
                    ...
                    match &expr.expr {
                        ExpressionType::Traversal(trav) => {
                            ...
                            let var_name = match &trav.start {
                                crate::sparrowc::parser::types::StartNode::Identifier(id) => {
                                    id.clone()
                                }
                                _ => "unknown".to_string(),
                            };
                            ...
                            if let Some(step) = trav.steps.first() {
                                if let crate::sparrowc::parser::types::StepType::Object(obj) =
                                    &step.step
                                {
                            ...
                            ReturnType::Expression(expr) => {
                                match &expr.expr {
                                    ...
                                    ExpressionType::Traversal(trav) => {
                                        ...
                                        let var_name = match &trav.start {
                                            crate::sparrowc::parser::types::StartNode::Identifier(
                                                id,
                                            ) => id.clone(),
                                            _ => "unknown".to_string(),
                                        };
                                        ...
                                        if let Some(step) = trav.steps.first() {
                                            if let crate::sparrowc::parser::types::StepType::Object(
                                                obj,
                                            ) = &step.step
                                            {
                                                if let Some(field) = obj.fields.first() {
    ...
    query.return_values.push((
        ...
        ReturnValue {
            ...
            fields: vec![],
            literal_value: Some(crate::sparrowc::generator::utils::GenRef::Std(format!(
                "json!({})",
                json_code
            ))),
        },
...
fn type_to_rust_string_and_fields(
    ...
    String,
    Vec<crate::sparrowc::generator::return_values::ReturnValueField>,
) {
    match (ty, should_collect) {
        ...
        (Type::Node(Some(label)), ShouldCollect::ToObj | ShouldCollect::No) => {
            ...
            let mut fields = vec![
                crate::sparrowc::generator::return_values::ReturnValueField::new(
                    "id".to_string(),
                    "ID".to_string(),
                )
                .with_implicit(true),
