...
use crate::sparrow_engine::traversal_core::ops::vectors::search::SearchVAdapter;
use crate::sparrow_engine::vector_core::vector::HVector;

type Filter = fn(&HVector, &RoTxn) -> bool;

/// Setup storage for concurrent testing
fn setup_concurrent_storage() -> (Arc<SparrowGraphStorage>, TempDir) {
                    ...
                    .collect::<Result<Vec<_>, _>>()
                {
                    Ok(results) => {
                        ...
                        for (i, result) in results.iter().enumerate() {
                            if let crate::sparrow_engine::traversal_core::traversal_value::TraversalValue::Vector(v) = result {
                                assert!(
                                    v.distance.is_some(),
