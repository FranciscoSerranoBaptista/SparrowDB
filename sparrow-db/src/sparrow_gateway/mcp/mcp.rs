...
use crate::{
    ...
    sparrow_gateway::mcp::tools::{EdgeType, FilterTraversal, Order, ToolArgs, execute_query_chain},
    protocol::{Format, Request, Response},
    utils::id::v6_uuid,
};
use bumpalo::Bump;
use sparrow_macros::mcp_handler;
...
pub struct McpBackend {
    pub db: Arc<SparrowGraphStorage>,
}

impl McpBackend {
    pub fn new(db: Arc<SparrowGraphStorage>) -> Self {
        Self { db }
    }
}
...
pub fn search_keyword(input: &mut MCPToolInput) -> Result<Response, GraphError> {
    use crate::sparrow_engine::traversal_core::ops::{bm25::search_bm25::SearchBM25Adapter, g::G};
...
pub fn search_vector_text(input: &mut MCPToolInput) -> Result<Response, GraphError> {
    ...
    let results = G::new(storage, &txn, &arena)
        .search_v::<fn(&crate::sparrow_engine::vector_core::vector::HVector, &Txn) -> bool, _>(
            query_vec_arena,
            k_value,
            label_arena,
            None,
