    ...
    AddEAdapter<'db, 'arena, 'txn, 's> for RwTraversalIterator<'db, 'arena, 'txn, I>
{
    ...
    > {
        ...
        // The value is empty
        let out_edge_key =
            SparrowGraphStorage::out_edge_key(from_node, &label_hash, to_node, edge.id);
        match self
            .txn
        ...
        let in_edge_key = SparrowGraphStorage::in_edge_key(to_node, &label_hash, from_node, edge.id);
        match self
            .txn
