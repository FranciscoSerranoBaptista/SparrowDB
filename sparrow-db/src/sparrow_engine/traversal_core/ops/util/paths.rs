...
type H = fn(&crate::utils::items::Node) -> Result<f64, crate::sparrow_engine::types::GraphError>;
        ...
        fn bfs_shortest_path(
            ...
            while let Some(current_id) = queue.pop_front() {
                let out_prefix = self.edge_label.map_or_else(
                    ...
                    |label| {
                        SparrowGraphStorage::out_edge_key(&current_id, &hash_label(label, None))
                            .to_vec()
                    },
                );
                ...
                for result in iter {
                    ...
                    let (edge_id, to_node) = match SparrowGraphStorage::unpack_adj_edge_data(value) {
        ...
        fn dijkstra_shortest_path(
            ...
            }) = heap.pop()
            {
                ...
                for result in iter {
                    ...
                    let (edge_id, to_node) =
                        SparrowGraphStorage::unpack_adj_edge_data(value).unwrap(); // TODO: handle error
        ...
        fn bfs_shortest_path(
            ...
            while let Some(current_id) = queue.pop_front() {
                ...
                let out_prefix = self.edge_label.map_or_else(
                    || current_id.to_be_bytes().to_vec(),
                    |label| {
                        SparrowGraphStorage::out_edge_key_prefix(current_id, &hash_label(label, None))
                            .to_vec()
                    },
                );
                ...
                while let Some(key) = iter.key() {
                    let (_, _, to_node_id, edge_id) =
                        SparrowGraphStorage::unpack_adj_edge_key(key).unwrap();
