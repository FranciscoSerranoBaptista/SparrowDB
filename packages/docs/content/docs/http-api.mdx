---
title: HTTP API
description: Complete reference for every SparrowDB HTTP endpoint — compiled queries, built-in operations, token management, and diagnostics.
---
...
When at least one token exists in the store, every request must include:
...
Accepts the HelixDB JSON DSL and translates it to SparrowDB storage operations.
...
All three endpoints require the caller to hold an `admin` token (or for the instance to have no tokens yet).

### `GET /tokens`

List all tokens. Returns records only — raw token strings are never stored.
...
Create a new named token. The raw token string is returned **once** and never stored.
...
**Bootstrap:** when no tokens exist yet, this endpoint is callable without an `x-api-key` header.
