...
use crate::sparrowc::parser::types::{Content, HxFile, Schema, Source};
use location::HasLoc;
...
impl SparrowParser {
    pub fn parse_source(input: &Content) -> Result<Source, ParserError> {
        ...
        input.files.iter().try_for_each(|file| {
            ...
            source.source.push('\n');
            let pair = match SparrowParser::parse(Rule::source, &file.content) {
                Ok(mut pairs) => pairs
                    .next()
