use crate::sparrow_engine::traversal_core::SparrowGraphEngineOpts;
...
use crate::sparrow_gateway::{
    gateway::CoreSetter,
    router::router::{HandlerInput, SparrowRouter, IoContFn},
};
use crate::protocol::Format;
use crate::protocol::{SparrowError, Request, request::RequestType, response::Response};
use axum::body::Bytes;
use std::sync::Arc;
...
async fn test_process_request_success() {
    ...
    let router = Arc::new(SparrowRouter::new(Some(routes), None, None));
...
async fn test_route_query_not_found() {
    ...
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), SparrowError::NotFound { .. }));
}
...
async fn test_not_found_error_contains_request_name() {
    ...
    match result {
        Err(SparrowError::NotFound { name, .. }) => {
...
async fn test_writer_continuation_priority() {
    ...
    let router = Arc::new(SparrowRouter::new(Some(routes), None, Some(write_routes)));
