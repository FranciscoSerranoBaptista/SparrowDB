use crate::sparrow_engine::traversal_core::SparrowGraphEngine;
...
use crate::sparrow_gateway::{
    gateway::CoreSetter,
    router::router::{HandlerInput, SparrowRouter},
};
...
fn create_test_pool(
    ...
    >,
) -> (WorkerPool, Arc<SparrowGraphEngine>, TempDir) {
    let (graph, temp_dir) = create_test_graph();
    let router = Arc::new(SparrowRouter::new(routes, None, None));
    let rt = Arc::new(
