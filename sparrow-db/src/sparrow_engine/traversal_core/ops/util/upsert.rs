...
fn update_secondary_index(
    bm25: &(
        ...
        crate::sparrow_engine::types::SecondaryIndex,
    ),
    ...
    match secondary_index {
        crate::sparrow_engine::types::SecondaryIndex::Unique(_) => db
            .put_with_flags(txn, PutFlags::NO_OVERWRITE, &serialized, &id)
            .map_err(|_| GraphError::DuplicateKey(key.to_string()))?,
        crate::sparrow_engine::types::SecondaryIndex::Index(_) => db.put(txn, &serialized, &id)?,
        crate::sparrow_engine::types::SecondaryIndex::None => unreachable!(),
    }
    Ok(())
}
...
fn update_bm25_node_doc(
    bm25: &HBM25Config,
    txn: &mut crate::sparrow_engine::traversal_core::WTxn<'_>,
    node_id: u128,
    ...
    > {
        ...
        let existing_edge: Result<Option<Edge>, GraphError> = (|| {
            ...
            for item in iter {
                ...
                let (edge_id, node_id) = SparrowGraphStorage::unpack_adj_edge_data(data)?;
                if node_id == to_node {
                            ...
                            for (k, v) in props.iter() {
                                ...
                                match secondary_index {
                                    crate::sparrow_engine::types::SecondaryIndex::Unique(_) => db
                                        ...
                                        .map_err(|_| GraphError::DuplicateKey(k.to_string()))?,
                                    crate::sparrow_engine::types::SecondaryIndex::Index(_) => {
                                        db.put(self.txn, &v_serialized, &vector.id)?
                                    }
                                    crate::sparrow_engine::types::SecondaryIndex::None => {
                                        unreachable!()
                                    }
                                }
                            }
