...
pub mod lmdb {
    ...
    pub struct SparrowGraphStorage {
        pub graph_env: Env,
        ...
        pub fn new(
            ...
            if let Some(indexes) = config.get_graph_config().secondary_indices {
                for index in indexes {
                    match index {
                        SecondaryIndex::Unique(name) => secondary_indices.insert(
                            name.clone(),
                            (
                                graph_env
                                    .database_options()
                        ...
                        SecondaryIndex::Index(name) => secondary_indices.insert(
                            name.clone(),
                            (
                                graph_env
                                    .database_options()
    ...
    impl DBMethods for SparrowGraphStorage {
        ...
        fn create_secondary_index(&mut self, name: &str) -> Result<(), GraphError> {
            // TODO: restore Unique vs Index distinction when secondary index semantics are ported
    ...
    impl StorageMethods for SparrowGraphStorage {
        #[inline]
        fn get_node<'arena>(
            &self,
            txn: &RoTxn,
