    ...
    > {
        let iter = self
            ...
            .filter_map(move |item| {
                ...
                let prefix = SparrowGraphStorage::in_edge_key(
                    &match item {
                        Ok(item) => item.id(),
                        Err(_) => return None,
                    },
                ...
                match self.storage.in_edges_db.get_duplicates(self.txn, &prefix) {
                    Ok(Some(iter)) => Some(iter.filter_map(move |item| {
                        if let Ok((_, value)) = item {
                            let (_, item_id) = match SparrowGraphStorage::unpack_adj_edge_data(value)
                            {
                                Ok(data) => data,
                                Err(e) => {
                ...
                match self.storage.in_edges_db.get_duplicates(self.txn, &prefix) {
                    Ok(Some(iter)) => Some(iter.filter_map(move |item| {
                        if let Ok((_, data)) = item {
                            let (_, item_id) = match SparrowGraphStorage::unpack_adj_edge_data(data) {
                                Ok(data) => data,
                                Err(e) => {
                                    println!("Error unpacking edge data: {e:?}");
                                    return Some(Err(e));
    ...
    > {
        ...
        let iter = self
            ...
            .filter_map(move |item| {
                ...
                let prefix = SparrowGraphStorage::in_edge_key_prefix(node_id, &edge_label_hash);
