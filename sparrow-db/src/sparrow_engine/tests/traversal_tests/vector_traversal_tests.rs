...
fn test_drop_vector_removes_edges() {
    ...
    Drop::drop_traversal(
        vectors
            .into_iter()
            .map(Ok::<_, crate::sparrow_engine::types::GraphError>),
        storage.as_ref(),
        &mut txn,
    )
    .unwrap();
...
fn test_v_from_type_without_vector_data() {
    ...
    match &results[0] {
        crate::sparrow_engine::traversal_core::traversal_value::TraversalValue::VectorNodeWithoutVectorData(v) => {
...
fn test_v_from_type_with_properties() {
    ...
    if let crate::sparrow_engine::traversal_core::traversal_value::TraversalValue::VectorNodeWithoutVectorData(v) = &results[0] {
...
fn test_v_from_type_after_migration() {
    ...
    fn clear_metadata(
        storage: &mut crate::sparrow_engine::storage_core::SparrowGraphStorage,
    ) -> Result<(), crate::sparrow_engine::types::GraphError> {
    ...
    for result in &results_without_data {
        if let crate::sparrow_engine::traversal_core::traversal_value::TraversalValue::VectorNodeWithoutVectorData(v) = result {
