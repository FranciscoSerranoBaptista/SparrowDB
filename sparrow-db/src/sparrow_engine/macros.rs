pub mod macros {
    ...
    /// use sparrow_db::protocol::filterable::Filterable;
    ...
    macro_rules! node_matches {
        ($key:expr, $value:expr) => {
            |node: &sparrow_db::protocol::items::Node| {
                if let Some(val) = node.get_property($key) {
                    if let sparrow_db::protocol::value::Value::String(val) = &val {
                        Ok(*val == $value)
                    } else {
                        Err(sparrow_db::sparrow_engine::types::GraphError::from(
    ...
    macro_rules! edge_matches {
        ($key:expr, $value:expr) => {
            |edge: &sparrow_db::protocol::items::Edge| {
