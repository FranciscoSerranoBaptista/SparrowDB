use crate::sparrow_engine::{
    storage_core::{
        SparrowGraphStorage, StorageConfig, storage_methods::DBMethods, version_info::VersionInfo,
    },
...
fn setup_test_storage() -> (SparrowGraphStorage, TempDir) {
...
fn test_node_key() {
    let id = 12345u128;
    let key = SparrowGraphStorage::node_key(&id);
    assert_eq!(*key, id);
}
...
fn test_edge_key() {
    let id = 67890u128;
    let key = SparrowGraphStorage::edge_key(&id);
    assert_eq!(*key, id);
}
...
fn test_out_edge_key() {
    ...
    let key = SparrowGraphStorage::out_edge_key(&from_node_id, &label);
...
fn test_in_edge_key() {
    ...
    let key = SparrowGraphStorage::in_edge_key(&to_node_id, &label);
...
fn test_out_edge_key_deterministic() {
    ...
    let key1 = SparrowGraphStorage::out_edge_key(&from_node_id, &label);
    let key2 = SparrowGraphStorage::out_edge_key(&from_node_id, &label);
...
fn test_in_edge_key_deterministic() {
    ...
    let key1 = SparrowGraphStorage::in_edge_key(&to_node_id, &label);
    let key2 = SparrowGraphStorage::in_edge_key(&to_node_id, &label);
...
fn test_pack_edge_data() {
    ...
    let packed = SparrowGraphStorage::pack_edge_data(&edge_id, &node_id);
...
fn test_unpack_adj_edge_data() {
    ...
    let (unpacked_edge_id, unpacked_node_id) =
        SparrowGraphStorage::unpack_adj_edge_data(&packed).unwrap();
...
fn test_unpack_adj_edge_data_invalid_length() {
    ...
    let _ = SparrowGraphStorage::unpack_adj_edge_data(&invalid_data);
}
...
fn test_out_edge_key_different_labels_produce_different_keys() {
    ...
    let key1 = SparrowGraphStorage::out_edge_key(&node_id, &label1);
    let key2 = SparrowGraphStorage::out_edge_key(&node_id, &label2);
...
fn test_out_edge_key_different_nodes_produce_different_keys() {
    ...
    let key1 = SparrowGraphStorage::out_edge_key(&node1, &label);
    let key2 = SparrowGraphStorage::out_edge_key(&node2, &label);
...
fn test_in_edge_key_different_labels_produce_different_keys() {
    ...
    let key1 = SparrowGraphStorage::in_edge_key(&node_id, &label1);
    let key2 = SparrowGraphStorage::in_edge_key(&node_id, &label2);
...
fn test_out_and_in_edge_keys_same_for_same_node_and_label() {
    ...
    let out_key = SparrowGraphStorage::out_edge_key(&node_id, &label);
    let in_key = SparrowGraphStorage::in_edge_key(&node_id, &label);
...
fn test_pack_edge_data_different_edge_ids_produce_different_data() {
    ...
    let packed1 = SparrowGraphStorage::pack_edge_data(&edge1, &node_id);
    let packed2 = SparrowGraphStorage::pack_edge_data(&edge2, &node_id);
...
fn test_pack_edge_data_different_node_ids_produce_different_data() {
    ...
    let packed1 = SparrowGraphStorage::pack_edge_data(&edge_id, &node1);
    let packed2 = SparrowGraphStorage::pack_edge_data(&edge_id, &node2);
...
fn test_unpack_adj_edge_data_short_slice_panics() {
    ...
    let _ = SparrowGraphStorage::unpack_adj_edge_data(&short_data);
}
...
fn test_unpack_adj_edge_data_empty_slice_panics() {
    ...
    let _ = SparrowGraphStorage::unpack_adj_edge_data(&empty_data);
}
...
fn test_unpack_adj_edge_data_16_bytes_panics() {
    ...
    let _ = SparrowGraphStorage::unpack_adj_edge_data(&partial_data);
}
...
fn test_pack_unpack_preserves_byte_order() {
    ...
    let packed = SparrowGraphStorage::pack_edge_data(&edge_id, &node_id);
    let (unpacked_edge, unpacked_node) = SparrowGraphStorage::unpack_adj_edge_data(&packed).unwrap();
...
fn test_out_edge_key_preserves_node_id_byte_order() {
    ...
    let key = SparrowGraphStorage::out_edge_key(&node_id, &label);
...
fn insert_node(storage: &SparrowGraphStorage, node: &Node) {
    let mut txn = storage.graph_env.write_txn().unwrap();
    let bytes = node.to_bincode_bytes().unwrap();
    storage
        .nodes_db
        .put(&mut txn, SparrowGraphStorage::node_key(&node.id), &bytes)
        .unwrap();
    txn.commit().unwrap();
}

fn insert_edge(storage: &SparrowGraphStorage, edge: &Edge) {
    let mut txn = storage.graph_env.write_txn().unwrap();
    let bytes = edge.to_bincode_bytes().unwrap();
    storage
        .edges_db
        .put(&mut txn, SparrowGraphStorage::edge_key(&edge.id), &bytes)
        .unwrap();

    // Insert into out_edges_db
    let label_hash = hash_label(edge.label, None);
    let out_key = SparrowGraphStorage::out_edge_key(&edge.from_node, &label_hash);
    let edge_data = SparrowGraphStorage::pack_edge_data(&edge.id, &edge.to_node);
    storage
        .out_edges_db
    ...
    let in_key = SparrowGraphStorage::in_edge_key(&edge.to_node, &label_hash);
    let in_edge_data = SparrowGraphStorage::pack_edge_data(&edge.id, &edge.from_node);
    storage
        .in_edges_db
...
fn test_drop_edge_verifies_out_edges_db_cleanup() {
    ...
    let out_key = SparrowGraphStorage::out_edge_key(&node1.id, &label_hash);
...
fn test_drop_edge_verifies_in_edges_db_cleanup() {
    ...
    let txn = storage.graph_env.read_txn().unwrap();
    let label_hash = hash_label("LINK", None);
    let in_key = SparrowGraphStorage::in_edge_key(&node2.id, &label_hash);
