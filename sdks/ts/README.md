...
TypeScript query DSL for SparrowDB. This package builds the same JSON AST shape as the Rust `sparrow-sdk` crate.
...
```ts
import { defineParams, g, param, readBatch } from "sparrow-sdk";
...
| Rust | TypeScript |
|------|------------|
| `#[register]` | `registerRead(...)` or `registerWrite(...)` |
| Rust function parameters | `defineParams(...)` |
| Rust parameter expressions | direct `params` properties |
